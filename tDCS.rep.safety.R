  ...
  # level <- 1
  # output <- safetytDCS(filename,AE.type,level)
  ...
  # Make sure AE type is in lower case
  ...
  if(is.element(1,level)){
    ...
    # Plot
    ccharge.max <- max(dat$ccharge[index]) + 5
    preds <- predict(res,newmods=c(0:ccharge.max),transf=exp)
    ...
    plot(metareg.dat$ccharge, exp(metareg.dat$yi), pch=19, cex=size, 
         ...
         las=1, bty="l", log="y",ylim = c(ymin,ymax), xlim = c(0,(ccharge.max)))
  ...
  if(is.element(2,level)){
    ...
    wi <- 1/sqrt(metareg.dat$vi)
    size <- 2 + 5.0 * (wi - min(wi))/(max(wi) - min(wi))
    colours <- brewer.pal(n = 6, name = "Dark2")
    colours <- cols[metareg.dat1$group]
  ...
  print(session.res);print(incidence.res);print(study.res)
