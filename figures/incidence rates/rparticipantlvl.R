...
col = colours, xlab="Cumulative Charge", ylab="Odd's Ratio",
las=1, bty="l", log="y",ylim = c(0.2,20),xlim=c(2,47)) #ylim = c(0.1, 6)
...
#offset <- 
text(dat1$ccharge[ids],exp(dat1$yi)[ids],dat1$author[ids],cex =.9, pos = pos)
...
# # Size = 600 x 600
# Discomfort(S4) - ylim = c(0.15,21); legend (22,21)
# Fatigue(S10) - ylim = c(0.3,10); legend(26,12)
# Headache(S7) - ylim = c(0.1,10); legend(26,0.6)
# Paraesthesia(S12) - ylim = c(0.2,20);

# paraesthesia specific
# colours <- cols[dat1$type]
# leg.txt <- c("Tingling", "Itching", "Burning")
# legend(20,20, leg.txt, pch = 19, col = cols, bty = "n", cex = 1, pt.cex = 2, xjust = 0, title = "Paraesthesia",title.adj=0)
